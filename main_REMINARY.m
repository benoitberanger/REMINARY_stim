function main_REMINARY(hObject, ~)
% main_REMINARY is the main program, calling the different tasks and
...
switch get(hObject,'Tag')
    ...
    otherwise
        error('REMINARY:TaskSelection','Error in Task selection')
end
...
%     otherwise
%         warning('REMINARY:ModeSelection','Error in Environement selection')
% end
%
...
switch get(get(handles.uipanel_SaveMode,'SelectedObject'),'Tag')
    ...
    case 'radiobutton_NoSave'
        SaveMode = 'NoSave';
    otherwise
        warning('REMINARY:SaveSelection','Error in SaveMode selection')
end
...
switch get(get(handles.uipanel_OperationMode,'SelectedObject'),'Tag')
    ...
    otherwise
        warning('REMINARY:ModeSelection','Error in Mode selection')
end
...
if isempty(SubjectID)
    error('REMINARY:SubjectIDLength','\n SubjectID is required \n')
end
...
%         Side = 'Right';
%     otherwise
%         warning('REMINARY:LeftRight','Error in LeftRight')
% end
%
% S.Side = Side;
...
switch get(get(handles.uipanel_EyelinkMode,'SelectedObject'),'Tag')
    ...
    case 'radiobutton_EyelinkOn'
        ...
        if isempty(status)
            error('REMINARY:EyelinkToolbox','no ''Eyelink.m'' detected in the path')
        end
        
        % Save mode ?
        if strcmp(S.SaveMode,'NoSave')
            error('REMINARY:SaveModeForEyelink',' \n ---> Save mode should be turned on when using Eyelink <--- \n ')
        end
    ...
    otherwise
        ...
        warning('REMINARY:EyelinkMode','Error in Eyelink mode')
...
switch get(handles.checkbox_WindowedScreen,'Value')
    ...
    case 0
        WindowedMode = 'Off';
    otherwise
        warning('REMINARY:WindowedScreen','Error in WindowedScreen')
