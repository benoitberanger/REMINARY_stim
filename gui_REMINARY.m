function varargout = gui_REMINARY
% SCHIRANG_GUI is the function that creates (or bring to focus) REMINARY GUI.
% Then, main_REMINARY is always called to start each task. It is the
% "main" program.
...
if ~isempty(figPtr) % Figure exists so brings it to the focus
    ...
    p_tk = Object_Xpos_Xwidth_dispatcher([ 1 1 ],p_tk);
    
    % ---------------------------------------------------------------------
    % Pushbutton : TRAINING
    
    p_tk.count  = p_tk.count + 1;
    b_TRAINING.x   = p_tk.xpos(p_tk.count);
    b_TRAINING.y   = buttun_y;
    b_TRAINING.w   = p_tk.xwidth(p_tk.count);
    b_TRAINING.h   = buttun_h;
    b_TRAINING.tag = 'pushbutton_TRAINING';
    handles.(b_TRAINING.tag) = uicontrol(handles.uipanel_Task,...
        'Style','pushbutton',...
        'Units', 'Normalized',...
    ...
    b_MRI.y   = buttun_y;
    b_MRI.w   = p_tk.xwidth(p_tk.count);
    b_MRI.h   = buttun_h;
